...
$(".setting").click(function () {
  $(".setting-side").toggle(effect, options, duration);
});
...
$.post("../class/display/display", { key: "display_item_borrow" })
  .done((data) => {
    if (!data) return;
    ...
    const options = JSON.parse(data)
      .map(
        (x) =>
          `<option value="${x.id}">${x.deviceid} - ${x.category} - ${x.brand} - [${x.stockleft} in stock] - ${x.status}</option>`,
      )
      .join("");
  ...
  .fail((xhr) => console.log(xhr.statusText));
...
$(document).ready(function () {
  $.ajax({
    ...
    $("#dueBorrow").text(data);
    $("#dueBadge").text(data).attr("data-count", data);
  });
});
