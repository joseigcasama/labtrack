// edit room
table_category.on("click", "a.category-edit", function (e) {
  e.preventDefault();
  ...
  $(".frm_editcategory").submit(function (e) {
    ...
    if (a_name == "") {
      toastr.warning("Fields is empty!");
    } else {
...
table_room.on("click", "a.edit-room", function (e) {
  ...
  var rm_id = data[5];
  $('input[name="edit_rm_name"]').val(rm_name);
  $('input[name="edit_rm_time"]').val(rm_due);
  $('input[name="edit_rm_id"]').val(rm_id);
  ...
  $(".frm_editroom").submit(function (e) {
    e.preventDefault();
    var a_name = $('input[name="edit_rm_name"]').val();
    var edit_rm_time = $('input[name="edit_rm_time"]').val();

    if (edit_rm_time == "" || a_name == "") {
      toastr.warning("Fields is empty!");
    } else {
...
// return
tbl_borrow.on("click", "a.edit-return", function (e) {
  e.preventDefault();

  $(".return-side").toggle(effect, options, duration);
  var data = tbl_borrow.row($(this).parents("tr")).data();
  var item_id = data[0];
  var item_stock_id = data[8];
  $('input[name="edit_item_id"]').val(item_id);
  $('input[name="stock_id"]').val(item_stock_id);

  $(".cancel_return_item").click(function () {
    $(".return-side").toggle(effect, options, duration);
  });

  $(".frm_return").submit(function (e) {
    e.preventDefault();
    var e_status = $('input[name="e_status"]').val();
    var remarks = $('input[name="remarks"]').val();

    if (remarks == "" || e_status == "") {
      toastr.warning("Fields is empty!");
    } else {
      var token = $(this).serialize() + "&key=return_item";
      $.ajax({
        type: "POST",
        url: "../class/edit/edit",
        data: token,
...
tbl_borrow_pending.on("click", "button.btn-accept", function (e) {
  e.preventDefault();
  ...
  $.ajax({
    ...
    if (data > 0) {
      ...
      tbl_borrow_pending.ajax.reload(null, false);
    } else {
