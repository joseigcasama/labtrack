...
$(".install").submit(function (e) {
  e.preventDefault();
  var datas = $(this).serialize() + "&key=install";
  ...
  $.ajax({
    ...
    .done(function (data) {
      ...
      if (data == 1) {
        toastr.success("System Successfully Configure", "Redirecting page");
        setTimeout(function () {
          window.location = "../index";
        }, 3000);
      } else {
        toastr.error("Failed to Configure");
      }
    })
    .fail(function (data) {
      console.log(data);
...
$(".frm_addequipment").on("submit", function (e) {
  e.preventDefault();

  const $form = $(this);
  const formData = new FormData(this); // auto include inputs + file
  ...
  $.ajax({
    ...
    .done((data) => {
      ...
      toastr[success ? "success" : "error"](
        success ? "Item added." : "Failed to add item",
      );
