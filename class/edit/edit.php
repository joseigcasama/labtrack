<?php
	...
	*/
	class edit
	{
		...
		public function edit_category($edit_rm_name,$edit_rm_id)
		{
			global $conn;
			session_start();
			$sql = $conn->prepare('UPDATE category SET description = ? WHERE id = ?');
			$sql->execute(array($edit_rm_name,$edit_rm_id));
			$count = $sql->rowCount();
			if($count > 0){
				echo "1";
			}else{
		...
		public function edit_room($edit_rm_name,$edit_rm_time,$edit_rm_id)
		{
			global $conn;
			...
			$sql = $conn->prepare('UPDATE room SET rm_name = ?, timelimit = ? WHERE id = ?');
			$sql->execute(array($edit_rm_name,$due,$edit_rm_id));
			$count = $sql->rowCount();
			if($count > 0){
				echo "1";
			}else{
		...
		public function return_item($e_status,$remarks,$edit_item_id,$date,$stock_id)
		{
			global $conn;
			...
			// $select = $conn->prepare('SELECT * FROM borrow WHERE id = ');
			// $select->execute(array($edit_item_id));
	  ...
	  //  	 	$rowcou = $insert->rowCount();
			...
			// $equip = $fetch['stock_id'];
			if ($e_status == 1) {
				$sql = $conn->prepare('UPDATE borrow SET status= ?,date_return = ?, e_remarks = ? WHERE borrowcode = ?;
								       UPDATE item SET i_status= ? WHERE id = ?;
								       UPDATE item_stock SET items_stock = (items_stock + ?) WHERE id = ?  ');
				$sql->execute(array($e_status,$today,$remarks,$edit_item_id,$e_status,$edit_item_id,1,$stock_id));
			}elseif ($e_status == 3 || $e_status == 4) {
				$sql = $conn->prepare('UPDATE borrow SET status= ?,date_return = ?, e_remarks = ? WHERE borrowcode = ?;
								   	  UPDATE item SET i_status= ? WHERE id = ? ');
				$sql->execute(array($e_status,$today,$remarks,$edit_item_id,$e_status,$edit_item_id));
			}
			$count = $sql->rowCount(); 
	...
	switch ($key) {
		...
		case 'return_item';
		$e_status = strtolower($_POST['e_status']);
		$date = $_POST['date'];
		$remarks = $_POST['remarks'];
		$stock_id = $_POST['stock_id'];
		$edit_item_id = $_POST['edit_item_id'];
		$edit->return_item($e_status,$remarks,$edit_item_id,$date,$stock_id);
		break;

		case 'edit_category';
		$edit_rm_name = $_POST['edit_rm_name'];
		$edit_rm_id = $_POST['edit_rm_id'];
		$edit->edit_category($edit_rm_name,$edit_rm_id);
		break;

		case 'edit_room';
		$edit_rm_name = $_POST['edit_rm_name'];
		$edit_rm_time = $_POST['edit_rm_time'];
		$edit_rm_id = $_POST['edit_rm_id'];
		$edit->edit_room($edit_rm_name,$edit_rm_time,$edit_rm_id);
		break;
